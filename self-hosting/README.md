...
- Установлена java
- **Java 21+** (OpenJDK 21 (именно этой версии)) и **Gradle** (для сборки бекендов, или используйте Gradle Wrapper)
...
После запуска PostgresSQL (psql) необходимо заполнить базу данными, нужно запустить скрипт "databse.sh" или "database.bat"
...
В конфиге для llm можно настроить:
- модель (можно использовать только те модели, которые доступны для скачивания в сервисе ollama (нужно их предварительно скачать));
- системный контекст для ИИ

После любых изменений конфигов, сервисы нужно перезапустить. Это можно сделать коммандой:
